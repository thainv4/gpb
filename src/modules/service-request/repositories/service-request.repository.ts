...
SELECT 
  ...
  EXDEPT.BRANCH_ID EXECUTE_BRANCH_ID,
  EXBR.BRANCH_CODE EXECUTE_BRANCH_CODE,
  EXBR.BRANCH_NAME EXECUTE_BRANCH_NAME,
...
FROM
  ...
  LEFT JOIN HIS_DEPARTMENT EXDEPT ON HSR.EXECUTE_DEPARTMENT_ID = EXDEPT.ID
  LEFT JOIN HIS_BRANCH EXBR ON EXDEPT.BRANCH_ID = EXBR.ID
WHERE
  A.IS_DELETE = 0
  AND A.IS_NO_EXECUTE is null`;
    ...
    const mapped: ServiceRequestView[] = rows.map((r) => ({
      ...
      executeBranchId: r.EXECUTE_BRANCH_ID,
      executeBranchCode: r.EXECUTE_BRANCH_CODE,
      executeBranchName: r.EXECUTE_BRANCH_NAME,
